...
// 1. MaterialData 構造体の定義
struct MaterialData
{
    std::string textureFilePath;
};

// 2. mtl ファイルを読む関数の空枠（宣言とスケルトン）
    ...
    while (std::getline(file, line)) {
        ...
        if (identifier == "mtllib") {
            ...
            for (int32_t faceVertex = 0; faceVertex < 3; ++faceVertex) {
                ...
                // ★1. 右手系→左手系変換のため、位置と法線の X 座標を反転する
                position.x *= -1.0f;
                normal.x *= -1.0f;

                // ★2. いったん配列に保存しておく（まだ push_back しない）
                triangle[faceVertex] = { position, texcoord, normal };
            }

            // ★3. 頂点を逆順 (2 -> 1 -> 0) で登録することで、面の回り順（時計回り/反時計回り）を逆にする
...
public:
    ResourceObject(ID3D12Resource* resource = nullptr)
        : resource_(resource)
    {
    }
    ~ResourceObject() {
...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int) {
   ...
   //
    ...
    ComPtr<ID3D12Resource> vertexResourcePlane = CreateBufferResource(device.Get(), sizeof(VertexData) * modelData.vertices.size());
    ...
    ComPtr<ID3D12Resource> vertexResourceSphere = CreateBufferResource(device.Get(), sizeof(VertexData) * kNumSphereVertices);
    ...
    for (uint32_t lat = 0; lat < kSubdivision; ++lat) {
        ...
        for (uint32_t lon = 0; lon < kSubdivision; ++lon) {
            ...
            vertexDataSphere[index + 0].position = p0; vertexDataSphere[index + 0].texcoord = { u0, v0 }; vertexDataSphere[index + 0].normal = { p0.x, p0.y, p0.z };
            vertexDataSphere[index + 1].position = p1; vertexDataSphere[index + 1].texcoord = { u0, v1 }; vertexDataSphere[index + 1].normal = { p1.x, p1.y, p1.z };
            vertexDataSphere[index + 2].position = p2; vertexDataSphere[index + 2].texcoord = { u1, v0 }; vertexDataSphere[index + 2].normal = { p2.x, p2.y, p2.z };

            vertexDataSphere[index + 3].position = p1; vertexDataSphere[index + 3].texcoord = { u0, v1 }; vertexDataSphere[index + 3].normal = { p1.x, p1.y, p1.z };
            vertexDataSphere[index + 4].position = p3; vertexDataSphere[index + 4].texcoord = { u1, v1 }; vertexDataSphere[index + 4].normal = { p3.x, p3.y, p3.z };
            vertexDataSphere[index + 5].position = p2; vertexDataSphere[index + 5].texcoord = { u1, v0 }; vertexDataSphere[index + 5].normal = { p2.x, p2.y, p2.z };
        }
    }

    // 3. フラグの宣言
    bool drawSphere = false; // trueなら球体、falseならPlaneを描画
...
// こういうもんである
    ...
    UpdateSubresources(commandList.Get(), textureResource.Get(), intermediateResource.Get(), 0, 0, UINT(subresources.size()), subresources.data());
   ...
   DirectX::ScratchImage mipImages2 = LoadTexture(modelData.material.textureFilePath);
    ...
    DirectX::PrepareUpload(device.Get(), mipImages2.GetImages(), mipImages2.GetImageCount(), mipImages2.GetMetadata(), subresources2);
    uint64_t intermediateSize2 = GetRequiredIntermediateSize(textureResource2.Get(), 0, UINT(subresources2.size()));
    ComPtr<ID3D12Resource> intermediateResource2 = CreateBufferResource(device.Get(), intermediateSize2);

    UpdateSubresources(commandList.Get(), textureResource2.Get(), intermediateResource2.Get(), 0, 0, UINT(subresources2.size()), subresources2.data());

    // ResourceStateをGENERIC_READに変更する
    D3D12_RESOURCE_BARRIER barrier2{};
    barrier2.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
    barrier2.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
    barrier2.Transition.pResource = textureResource2.Get();
    ...
    commandList->Close();
    ID3D12CommandList* commandLists2[] = { commandList.Get() };
    ...
    while (msg.message != WM_QUIT) {
        ...
        if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
            ...
            // Spriteで useMonsterBall の切り替えを行いたい場合はここで行う
            commandList->SetGraphicsRootDescriptorTable(2, useMonsterBall ? textureSrvHandleGPU2 : textureSrvHandleGPU);

            commandList->SetGraphicsRootConstantBufferView(1, transformationMatrixResourceSprite->GetGPUVirtualAddress());
            commandList->DrawIndexedInstanced(6, 1, 0, 0, 0); // Spriteを描画したい場合はコメントアウト解除
    ...
    // WinAPIのハンドルだけはCloseHandleする
    CloseHandle(fenceEvent);

   CoUninitialize();

    // リークチェック（すべての ComPtr が破棄された後にチェックしたい場合は、スマートポインタのスコープに注意してください）
